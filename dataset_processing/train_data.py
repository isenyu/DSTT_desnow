...
class K12_dataset(data.Dataset):
    ...
    def __init__(self, root, crop_size, frame_num, step, apply_crop=True):
    ...
    def __getitem__(self, idx):
        ...
        #transform_params = self.get_transform_params(ip_frames1[0].size[0], ip_frames1[0].size[1])
    ...
    def apply_transform_stereo(self, ip_frame, gt_frame, ip_frame2, gt_frame2):
        width, height = ip_frame.size

        if width < self.crop_size or height < self.crop_size:
            raise Exception('Bad image size: {}'.format(ip_frame))

        # --- x,y coordinate of left-top corner --- #
        x, y = randrange(0, width - self.crop_size + 1), randrange(0, height - self.crop_size + 1)
        haze1_crop_img = ip_frame.crop((x, y, x + self.crop_size, y + self.crop_size))
        gt1_crop_img = gt_frame.crop((x, y, x + self.crop_size, y + self.crop_size))
        haze2_crop_img = ip_frame2.crop((x, y, x + self.crop_size, y + self.crop_size))
        gt2_crop_img = gt_frame2.crop((x, y, x + self.crop_size, y + self.crop_size))
        transform = Compose([ToTensor()])
        # gt_frame = Compose(ToTensor())
        ip_frame = transform(haze1_crop_img)
        gt_frame = transform(gt1_crop_img)
        ip_frame2 = transform(haze2_crop_img)
        gt_frame2 = transform(gt2_crop_img)
